...
export async function POST(req: Request) {
  ...
  // const test = new SerpAPI(process.env.SERPAPI_API_KEY, {
  //   as_sitesearch:
  //     "https://reddit.com https://wikipedia.org https://quora.com https://medium.com",
  //   as_dt: "e",
  // });

  // const test2 = await test.toJSON();
...
Do not add sources to the arguments.`;
  ...
  totalPromptTokens += enc.encode(prompt2).length;
  chain.run(prompt2, [handlers]).then(async (res) => {
    ...
    totalExecutionTokens += enc.encode(res).length;

    console.log("Total Prompt Tokens", totalPromptTokens);
    console.log("Total Execution Tokens", totalExecutionTokens);
    console.log("Total Tokens", totalPromptTokens + totalExecutionTokens);
